...
class SettingsScreen extends StatefulWidget {
  ...
  final HermesBridgeService? hermesBridgeService;
...
class _SettingsScreenState extends State<SettingsScreen>
  ...
  void initState() {
    ...
    _hermesRunning = widget.hermesBridgeService?.isRunning ?? false;
    _checkPermissions();
  }
  ...
  Future<void> _toggleHermesBridge(bool enable) async {
    final bridge = widget.hermesBridgeService;
    ...
    if (enable) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          ...
          const SnackBar(content: Text('Hermes bridge started on 127.0.0.1:8767')),
        );
      }
    } else {
      await bridge.stop();
      if (mounted) {
        ScaffoldMessenger.of(
          ...
          const SnackBar(content: Text('Hermes bridge stopped')),
        );
      }
    }
    ...
    if (mounted) {
      setState(() {
        _hermesRunning = bridge.isRunning;
      });
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Start a local HTTP bridge so a Hermes instance in Termux can control this device. '
            'Only listens on 127.0.0.1:8767 — same-device only.',
            style: Theme.of(context).textTheme.bodySmall,
          ),
          const SizedBox(height: 12),
          SwitchListTile(
            title: const Text('Enable Hermes Bridge'),
            subtitle: Text(
              _hermesRunning
                  ? 'Running on http://127.0.0.1:8767'
                  : 'Stopped — enable to let Hermes send commands',
              style: TextStyle(
                color: _hermesRunning ? Colors.green : Colors.grey,
                fontSize: 12,
              ),
            ),
            value: _hermesRunning,
            onChanged: widget.hermesBridgeService == null
                ? null
                : (val) => _toggleHermesBridge(val),
            contentPadding: EdgeInsets.zero,
          ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (!isRunning) ...[
                  const Text(
                    'Tap below to open Accessibility Settings, then find "ArcaneFlow Screen Control" and enable it.',
                    style: TextStyle(fontSize: 13),
                  ),
                  const SizedBox(height: 12),
                  OutlinedButton.icon(
